...
<div class="deliberations">
    ...
    echo $this->Form->create('Deliberation', array(
        'url' => array('controller' => 'deliberations', 'action' => 'sendToSignature', $seance_id),
        'type' => 'file'
    ));
    ?>
    <table style='width:100%'>
        ...
        foreach ($deliberations as $delib) {
            ...
            $options = array();
            if ($seance_id != null) {
                if ($delib['Deliberation']['etat'] >= 2
                    && (empty($delib['Deliberation']['parapheur_etat']) || $delib['Deliberation']['parapheur_etat'] == -1)
                    && empty($delib['Deliberation']['signee'])
                    && !empty($delib['Deliberation']['num_pref'])
                    && (in_array($delib['Deliberation']['etat'], array(3, 4)) || $delib['Deliberation']['Typeacte']['nature_id'] > 1)
                )
...
<style>
    #DeliberationSendToSignatureForm label {
        float: none;
        padding: 0;
        text-align: left;
        width: auto;
    ...
    #DeliberationSendToSignatureForm .sans-bordure {
        border-radius: 0;
        -moz-border-radius: 0;
        -webkit-border-radius: 0;
    }

    #DeliberationSendToSignatureForm .select2-container .select2-choice {
        border-radius: 0;
    }

    #DeliberationSendToSignatureForm .select-circuit {
        margin-bottom: 0;
        width: auto;
        max-width: 500px;
    }
