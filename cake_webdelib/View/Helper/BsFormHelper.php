		...
		if (!$date) {
			...
			if (isset($options['class'])) {
                                if($options['class']==false){
                                    //Correction pour les input image
                                    unset($options['class']);
                                }
                                else
 ...
 * - ... and more at http://www.malot.fr/bootstrap-datetimepicker/
 * fork of http://eternicode.github.io/bootstrap-datepicker/
 *
 * @param string $fieldName This should be "Modelname.fieldname"
 ...
 */
	public function datetimepicker($fieldName, $optionsDP = array(), $options = array()) {
		...
		if (is_array($fieldName)) {
			...
			$script = "$('.dp-container .input-daterange').datetimepicker({";
			...
			$out .= '<div class="input-daterange input-group" id="datetimepicker">';
			...
			foreach ($fieldName as $key => $field) {
				...
				$script .= 'var date_' . $key . ' = $(\'#' . parent::domId($field) . '\').datetimepicker(\'getDate\');
			...
			$out = '<div class="dtp-container">';
                        $options['type'] = 'text';
			$out .= $this->input($fieldName, $options);
			/*$options['id'] = Inflector::camelize($fieldName);
			$options['type'] = 'hidden';
			$out .= $this->input($fieldName, $options);*/
			$out .= '</div>';
                        //debug( $fieldName);
			...
			$script = "$('#".parent::domId()."').datetimepicker({";
			$script .= $this->__scriptDP($optionsDP) . '});';

			/*$script .= '.on(\'changeDate\', function() {
				var date = $(\'#' . parent::domId($fieldName) . '\').datetimepicker(\'getDate\');
				date.setHours(0, -date.getTimezoneOffset(), 0, 0);
				date = date.toISOString().slice(0,26).replace(\'T\', " ");
