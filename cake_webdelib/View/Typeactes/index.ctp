...
foreach ($typeactes as $typeacte) {
    echo $this->Bs->tableCells(array(
        ...
        $this->Bs->btn(null, array('controller' => 'typeactes', 'action' => 'delete', $typeacte['Typeacte']['id']), array('type' => 'danger', 'icon' => ' glyphicon glyphicon-trash', 'title' => 'Supprimer', 'class' => !$typeacte['Typeacte']['is_deletable'] ? 'disabled' : ''), 'Êtes vous sur de vouloir supprimer ' . $typeacte['Typeacte']['libelle'] . ' ?') .
        $this->Bs->close()
    ));
}
