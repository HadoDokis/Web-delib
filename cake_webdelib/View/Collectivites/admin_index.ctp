...
echo $this->Bs->tag('h3', 'Information de votre collectivité') .
 ...
 $this->Bs->cell(
    ...
    $this->Bs->btn( null, array(
        'controller' => 'collectivites', 'action' => 'edit'), array(
            'type' => 'primary', 'icon'=>'glyphicon glyphicon-edit', 'title' => 'Modifier')) .
    $this->Bs->btn( null, array(
        'controller' => 'collectivites', 'action' => 'setLogo'), array(
            'type' => 'default', 'icon'=>'glyphicon glyphicon-picture', 'title' => 'Changer de logo (page de connexion)')).
            $this->Bs->close()
    ).
$this->Bs->endTable();