...
if (isset($visas_retard) && !empty($visas_retard)) {
    foreach ($visas_retard as $visa) {
        ...
        echo $this->Html->link(
                $this->Html->tag("i", "", array("class" => "icon-repeat")), "/cakeflow/traitements/traiterDelegationsPassees/" . $visa["Visa"]['traitement_id'] . "/" . $visa["Visa"]['numero_traitement'] . '/traiter', array('escape' => false, "style" => "text-decoration:none;margin-right:5px;", 'title'=> 'Mettre à jour le statut de cette étape'));
        ?>');
    <?php
    }
}
