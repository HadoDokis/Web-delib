    ...
    callWithJQuery(function ($) {
                        ...
                        return {
                            ...
                            push: function (record) {
                                formatter = numberFormat({
                                    digitsAfterDecimal: 0,
                                    // prefix: '<a href="/tableau_bord/'+record['SV']+';'+record['SR']+'">',
                                    // suffix: "</a>",
                                    showZero: true
                                });
                                this.id = record['SV'] + 'A' + record['SR'];
                                this.url = record['url'];
                                this.Datelimite = record['date_limite'];
        ...
        pivotTableRenderer = function (pivotData, opts) {
            ...
            for (j in colAttrs) {
                ...
                //th.bind("click", showFilterList);
                //construction du tableau service validateur(ou autre) niveau 1,2,...
                th.innerHTML = c.replace(/\_/g, ' ');
                tr.appendChild(th);
                for (i in colKeys) {
                    ...
                    if (x !== -1) {
                        ...
                        // attribut du service validateur
                        /*th.id = */
                        th.nodeValue = colKeys;
            ...
            if (rowAttrs.length !== 0) {
                ...
                for (i in rowAttrs) {
                    ...
                    th = document.createElement("th");
                    th.className = "pvtAxisLabel";
                    //construction du tableau service redacteur(ou autre) niveau 1,2,...
                    th.innerHTML = r.replace(/\_/g, ' ');
                    tr.appendChild(th);
                }
                th = document.createElement("th");
            ...
            for (i in rowKeys) {
                ...
                //creation et attribution des valeurs des cases
                for (j in colKeys) {
                    ...
                    if(aggregator.Datelimite >= 1){
                        td.className = "pvtVal pvtValAlerte row" + i + " col" + j;
                        td.title = 'Vous avez '+aggregator.Datelimite+' projets en retard';//\n
                    } else{
                        td.className = "pvtVal row" + i + " col" + j;
                    }
                    
                    //affichage 0 a la plcae de rien
                    if (val === null) {
                        td.className += " pvtValnull";
                        td.innerHTML = '0';
                    }
                    else {
                        ...
                        td.onclick = function () {
                            //alert($( this ).attr('id'));
                            $.ajax({
                                url: aggregator.url + $(this).attr('id'), //url est celle de la dergniere cellule
                                beforeSend: function () {
                                    $("#view").html('');
                                },
        ...
        $.fn.pivotUI = function (input, inputOpts, overwrite, locale) {
            ...
            try {
                ...
                _fn = function (c) {
                    ...
                    attrElem = $("<li>").addClass("axis_" + i).append($("<span>").addClass('pvtAttr').text(c.replace(/\_/g, ' ')).data("attrName", c).append(triangleLink));
                    if (hasExcludedItem) {
                        attrElem.addClass('pvtFilteredAttribute');
                    }
                    colList.append(attrElem).append(valueList);
                    return attrElem.bind("click", showFilterList);
                };
                ...
                aggregator = $('<select style="display:none">').addClass('pvtAggregator').bind("change", function () {
                    return refresh();
                });
