...
// Constantes de la classe
    ...
    function commit($transactionSessionKey = self::TRANSACTION_SESSION_KEY) {
        ...
        if (!AppGestfichiers::_transactionEnCours($transactionSessionKey)) return;
            ...
            switch($rollback['action']) {
                case self::TRANSACTION_ACTION_DEL :
                    if ($rollback['type']==self::TRANSACTION_TYPE_FILE)
                        AppGestfichiers::_supprimeFichier($rollback['element']);
                    else
                        AppGestfichiers::_supprimeRepertoire($rollback['element']);
                    break;
                case self::TRANSACTION_ACTION_RENAME :
    ...
    function ajouteFichierTransaction($fileUri, $options=array()) {
        try {
            ...
            if ($options['session'] == self::TRANSACTION_ROLLBACK_KEY)
                AppGestfichiers::_addToRollback($options['transactionSessionKey'], $fileUri);
            else
                AppGestfichiers::_addToCommit($options['transactionSessionKey'], $fileUri);
        } catch(Exception $e) {
    ...
    function ajouteRepertoireTransaction($dirUri, $options=array()) {
        try {
            ...
            $dirUri = AppGestfichiers::formatDirName($dirUri);
            if (!is_dir($dirUri))
            ...
            if (!$options['exludeHimself']) {
                if ($options['session'] == self::TRANSACTION_ROLLBACK_KEY)
                    AppGestfichiers::_addToRollback($options['transactionSessionKey'], $dirUri, array('type'=>self::TRANSACTION_TYPE_DIR));
                else
                    AppGestfichiers::_addToCommit($options['transactionSessionKey'], $dirUri, array('type'=>self::TRANSACTION_TYPE_DIR));
            }
            ...
            foreach ($repDir as $fileDirUri) {
                if ($options['includeFiles'] && is_file($fileDirUri)) {
                    if ($options['session'] == self::TRANSACTION_ROLLBACK_KEY)
                        AppGestfichiers::_addToRollback($options['transactionSessionKey'], $fileDirUri);
                    else
                        AppGestfichiers::_addToCommit($options['transactionSessionKey'], $fileDirUri);
                }
                if ($options['recursive'] && is_dir($fileDirUri))
                    AppGestfichiers::ajouteRepertoireTransaction($fileDirUri, array(
    ...
    function creeFichier($fileUri, $fileContent, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            ...
            $rep = dirname($fileUri);
            if (!is_dir($rep))
                AppGestfichiers::creeRepertoire($rep, $transactionSessionKey);

            // écriture du fichier
            if (file_put_contents($fileUri, $fileContent)===false)
                throw new Exception(__('Echec de la création du fichier', true).' '.$fileUri);
            AppGestfichiers::_addToRollback($transactionSessionKey, $fileUri);
        } catch(Exception $e) {
            ...
            if (!is_dir($rep)) {
                ...
                foreach($repParts as $repPart) {
                    ...
                    if (!is_dir($repACreer)) {
                        ...
                        AppGestfichiers::_addToRollback($transactionSessionKey, $repACreer, array('type'=>self::TRANSACTION_TYPE_DIR));
                    }
                }
            }
    ...
    function copieFichier($ficSourceUri, $ficDestUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        ...
        try {
            ...
            if (!is_dir($repDest))
                AppGestfichiers::creeRepertoire($repDest, $transactionSessionKey);
            ...
            if (!is_file($ficDestUri))
                throw new Exception(__('Echec de la copie du fichier', true).' '.$ficSourceUri.' '.__('vers', true).' '.$ficDestUri);
            if (AppGestfichiers::calcHashFile($ficSourceUri, 'SHA256') != AppGestfichiers::calcHashFile($ficDestUri, 'SHA256')) {
                AppGestfichiers::_supprimeFichier($ficDestUri);
                throw new Exception(__('Empreinte du fichier', true).' : '.$ficSourceUri.' '.__('différente de l\'empreinte du fichier', true).' : '.$ficDestUri);
            }
            AppGestfichiers::_addToRollback($transactionSessionKey, $ficDestUri);
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    ...
    function copieRepertoire($repSourceUri, $repDestUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            // initialisations
            $repSourceUri = AppGestfichiers::formatDirName($repSourceUri);
            $repDestUri = AppGestfichiers::formatDirName($repDestUri);
            ...
            foreach($fichiersSource as $fileSourceUri) {
                ...
                if (is_file($fileSourceUri)) {
                    // copie
                    AppGestfichiers::copieFichier($fileSourceUri, $fileDestUri, $transactionSessionKey);
                } else {
                    // traitement des sous repertoires
                    AppGestfichiers::copieRepertoire($fileSourceUri, $fileDestUri, $transactionSessionKey);
                }
            }
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
    ...
    function deplaceFichier($ficSourceUri, $ficDestUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            AppGestfichiers::copieFichier($ficSourceUri, $ficDestUri, $transactionSessionKey);
            AppGestfichiers::supprimeFichier($ficSourceUri, $transactionSessionKey);
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    }
    ...
    function deplaceFichierTelecharge($tmpFicTeleUri, $ficDestUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            ...
            AppGestfichiers::ajouteFichierTransaction($ficDestUri, array('transactionSessionKey'=>$transactionSessionKey));
    ...
    function deplaceRepertoire($repSourceUri, $repDestUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            AppGestfichiers::copieRepertoire($repSourceUri, $repDestUri, $transactionSessionKey);
            AppGestfichiers::supprimeRepertoire($repSourceUri, $transactionSessionKey);
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    }
    ...
    function renommeFichier($fileUri, $newFileUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            ...
            AppGestfichiers::_addToRollback($transactionSessionKey, array('from'=>$newFileUri, 'to'=>$fileUri), array('action'=>AppGestfichiers::TRANSACTION_ACTION_RENAME));
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    }
    ...
    function supprimeFichier($fileUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            ...
            if (AppGestfichiers::_transactionEnCours($transactionSessionKey))
                AppGestfichiers::_addToCommit($transactionSessionKey, $fileUri);
            else
                AppGestfichiers::_supprimeFichier($fileUri);
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    }
    ...
    function supprimeRepertoire($repUri, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        ...
        try {
            // initialisation
            $repUri = AppGestfichiers::formatDirName($repUri);
            ...
            // gestion de la transaction
            if (AppGestfichiers::_transactionEnCours($transactionSessionKey))
                AppGestfichiers::_addToCommit($transactionSessionKey, $repUri, array('type'=>self::TRANSACTION_TYPE_DIR));
            ...
            foreach($fichiers as $fichier) {
                if (is_file($fichier)) {
                    // gestion de la transaction
                    if (AppGestfichiers::_transactionEnCours($transactionSessionKey))
                        AppGestfichiers::_addToCommit($transactionSessionKey, $fichier, array('type'=>self::TRANSACTION_TYPE_FILE));
                    else
                        AppGestfichiers::_supprimeFichier($fichier);
                } else {
                    // traitement des sous repertoires
                    AppGestfichiers::supprimeRepertoire($fichier, $transactionSessionKey);
                }
            }

            // gestion de la transaction
            if (!AppGestfichiers::_transactionEnCours($transactionSessionKey))
                AppGestfichiers::_supprimeRepertoire($repUri);
        } catch(Exception $e) {
            throw new Exception($e->getMessage());
        }
    }
    ...
    function createTmpDir($dirNameRoot) {
        ...
        $dirName = AppGestfichiers::formatDirName($dirNameRoot);
        ...
        try {
            AppGestfichiers::creeRepertoire($ret);
        } catch (Exception $e) {
            return false;
        }
        return $ret;
    ...
    function clearDir($dirName) {
        // initialisations
        $dirName = AppGestfichiers::formateRepertoire($dirName);
        ...
        while($fichier=readdir($ouverture)) {
            ...
            if (is_dir($curFichier)) {
                $r = AppGestfichiers::clearDir($curFichier);
                if (!$r) return false;
            } else {
                $r = @unlink($curFichier);
                if (!$r) return false;
    ...
    function checkHashFile($fileUri, $hashType, $hashValue) {
        $hash = AppGestfichiers::calcHashFile($fileUri, $hashType);
    ...
    function decompresse($archiveFileUri, $destDirName, $transactionSessionKey=self::TRANSACTION_SESSION_KEY) {
        try {
            // initialisation
            $destDirName = AppGestfichiers::formatDirName($destDirName);
            $erreurs = array();
            ...
            AppGestfichiers::ajouteRepertoireTransaction($destDirName, array('exludeHimself'=>true, 'transactionSessionKey'=>$transactionSessionKey));
