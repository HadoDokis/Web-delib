...
class InfosuplistedefsController extends AppController
{
    ...
    function index($infosupdefId)
    {
        ...
        if (empty($infosupdef)) {
            ...
            $sortie = true;
        } elseif (!in_array($infosupdef['Infosupdef']['type'], array('list','listmulti'))) {
        ...
        if ($sortie)
            $this->redirect(array('controller'=>'infosupdefs', 'action'=>'index'));
        else {
    ...
    function add($infosupId = 0)
    {
        ...
        if (empty($this->data)) {
            ...
            $infosupdef = $this->{$this->modelClass}->Infosupdef->find('first', array(
                'conditions' => array('Infosupdef.id' => $infosupId),
                'fields' => array('id', 'type', 'nom'),
                'recursive' => -1));
            if (empty($infosupdef)) {
                $this->Session->setFlash('Invalide id pour l\'information supplémentaire : édition impossible', 'growl', array('type' => 'erreur'));
                $redirect = array('controller'=>'infosupdefs', 'action'=>'index');
                $sortie = true;
            } elseif (!in_array($infosupdef['Infosupdef']['type'], array('list','listmulti'))) {
                $this->Session->setFlash('Cette information supplémentaire n\'est pas de type liste : édition impossible', 'growl', array('type' => 'erreur'));
                $redirect = array('controller'=>'infosupdefs', 'action'=>'index');
            ...
            $nb_liste = $this->Infosuplistedef->find('count', array(
                'conditions' => array('Infosuplistedef.infosupdef_id' => $infosupId),
                'recursive' => -1));
            ...
            if ($this->Infosuplistedef->save($this->data['Infosuplistedef'])) {
                ...
                $redirect = array('controller'=>'infosuplistedefs', 'action'=>'index', $this->data['Infosuplistedef']['infosupdef_id']);
                $sortie = true;
            } else {
    ...
    function changerOrdre($id = null, $suivant = true)
    {
        ...
        $this->redirect(array('controller'=>'infosuplistedefs', 'action'=>'index', $data['Infosuplistedef']['infosupdef_id']));
    }
