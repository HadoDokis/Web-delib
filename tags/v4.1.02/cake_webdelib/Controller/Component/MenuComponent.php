		...
		foreach($controllerList as $controllerFile) {
			if (strpos($controllerFile, 'Controller.php')>0) {
				...
				foreach($listeActions as $key => $action) {
					if ($this->_trouveAction( Inflector::camelize(str_replace('Controller.php','',$controllerFile)) . ':' . $action, $menuCtrlTree)) unset($listeActions[$key]);
				}
				// Ajout à la liste des menus-controleurs
				if (!empty($listeActions)) {
					$menuCtrlTree[] = array();
					$key = count($menuCtrlTree) - 1;
					$acosAlias = 'Module:'.Inflector::camelize(str_replace('Controller.php','',$controllerFile));
					$menuCtrlTree[$key]['title'] = 'Module:'.Inflector::camelize(str_replace('Controller.php','',$controllerFile));
					$menuCtrlTree[$key]['acosAlias'] = $acosAlias;
					...
					foreach($listeActions as $action) {
						$key++;
						$acosAlias = Inflector::camelize(str_replace('Controller.php','',$controllerFile)). ':' . $action;
