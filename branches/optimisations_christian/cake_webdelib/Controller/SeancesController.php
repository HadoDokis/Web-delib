...
//			$date = date('d/m/Y', strtotime($this->data['date']));
    ...
    function details($seance_id = null) {
        ...
        for ($i = 0; $i < count($deliberations); $i++) {
            ...
            $deliberations[$i]['Model']['id'] = $this->Typeseance->modeleProjetDelibParTypeSeanceId($seance['Seance']['type_id'], $deliberations[$i]['Deliberation']['etat']);
        }
        $this->set('seance', $seance);
...
//		$position =  $this->Deliberation->getPosition($deliberation_id, $seance_id);
        if (empty($this->data)) {
            ...
            if (($this->data['Deliberation']['etat'] == 3) && empty($deliberation['Deliberation']['num_delib'])) {
                ...
                $this->request->data['Deliberation']['num_delib'] = str_replace('#p#', $this->Deliberation->getPosition($deliberation_id, $seance_id), $this->data['Deliberation']['num_delib']);
            }
    ...
    function donnerAvis($deliberation_id, $seance_id) {
        ...
        $this->set('seances', $this->Seance->generateList(array(), $afficherTtesLesSeances, array_keys($this->Session->read('user.Nature')))
        );
...
//Obsolete
    ...
    protected function _generer($seance_id, $model_id, $url_retour) {
        ...
        $oTemplate = new GDO_ContentType("", $model['Model']['modele'], "application/vnd.oasis.opendocument.text", "binary", $model['Model']['content']);
