<h2>Retourner le projet à une étape précédente</h2>

<?php
echo $this->Form->create('Traitement', array('url' => array('controller' => 'deliberations', 'action' => 'retour', $delib_id)));
echo $this->Form->input('etape', array('label' => 'Etape du circuit', 'title' => "A quelle étape voulez vous retourner le projet ?", 'style' => 'width: auto; max-width: 100%;'));
echo '<br/>';
